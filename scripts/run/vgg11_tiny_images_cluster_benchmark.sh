...
run_dir="${VGG_RUN_DIR:-results/vgg11_tiny_images_cluster_$(date +%Y%m%d-%H%M%S)}"
mkdir -p "$run_dir"
...
if [[ "${VGG_CLUSTER_RUN_NOW:-0}" != "1" ]]; then
  echo "VGG11_TINY_IMAGES_CLUSTER_READY=YES"
  echo "VGG11_TINY_IMAGES_CLUSTER_DATASET=$dataset_dir"
  echo "VGG11_TINY_IMAGES_CLUSTER_COMMAND_FILE=$run_dir/cluster_command.sh"
...
echo "VGG11_TINY_IMAGES_CLUSTER_DONE=YES"
echo "VGG11_TINY_IMAGES_CLUSTER_DIR=$run_dir"
