...
echo "world_size,total_ms_with_comm,total_ms_without_comm,speedup_with_comm,speedup_without_comm,efficiency_with_comm,efficiency_without_comm,tile_grid,schedule" > "$csv"
...
for np in ${YOLO_P_LIST:-1 2 3}; do
  ...
  read -r with without tile schedule < <(python3 - "$out/summary.csv" <<'PY'
import csv, sys
with open(sys.argv[1], newline="", encoding="utf-8") as f:
    row = next(csv.DictReader(f))
print(row["total_ms_with_comm"], row["total_ms_without_comm"], row["tile_grid"], row["schedule"])
PY
)
  ...
  python3 - "$csv" "$np" "$with" "$without" "$baseline_with" "$baseline_without" "$tile" "$schedule" <<'PY'
import csv, sys
path, p, tw, tc, bw, bc, tile, schedule = sys.argv[1:]
p = int(p); tw = float(tw); tc = float(tc); bw = float(bw); bc = float(bc)
sw = bw / tw if tw else 0.0
sc = bc / tc if tc else 0.0
with open(path, "a", newline="", encoding="utf-8") as f:
    csv.writer(f).writerow([p, tw, tc, sw, sc, sw / p if p else 0.0, sc / p if p else 0.0, tile, schedule])
PY
done
