...
// Tensor layout used by the Method 2 benchmark.
// We keep it deliberately simple: channel-first C x H x W, contiguous in memory.
struct Tensor {
  int c = 0;
  int h = 0;
  int w = 0;
...
inline Tensor make_deterministic_input(int channels, int height, int width) {
  ...
  // Smooth deterministic values make serial/distributed comparison stable.
...
inline ConvWeights make_deterministic_weights(int layer_id, int cin, int cout, int kernel = 3) {
  ...
  // Small values avoid numeric blow-up after several VGG-style layers.
        ...
        for (int ic = 0; ic < weights.in_channels; ++ic) {
          for (int ky = 0; ky < 3; ++ky) {
            for (int kx = 0; kx < 3; ++kx) {
              ...
              // Same padding: points outside the feature map contribute zero.
