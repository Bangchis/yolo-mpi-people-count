...
static Config parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (key == "--source") {
            ...
            cfg.model = need_value(key);
        } else if (key == "--device") {
            cfg.device = need_value(key);
        } else if (key == "--imgsz") {
            cfg.imgsz = std::stoi(need_value(key));
        } else if (key == "--conf") {
            cfg.conf = std::stod(need_value(key));
        } else if (key == "--iou") {
            cfg.iou = std::stod(need_value(key));
        } else if (key == "--tile-grid") {
            cfg.tile_grid = need_value(key);
        } else if (key == "--overlap") {
        ...
        // Duplicate-removal controls used in postprocess.
            ...
            cfg.duplicate_center = std::stod(need_value(key));
        } else if (key == "--dedup-axis-overlap") {
            cfg.duplicate_axis_overlap = std::stod(need_value(key));
        } else if (key == "--dedup-gap") {
            cfg.duplicate_gap = std::stod(need_value(key));
        } else if (key == "--dedup-near-camera") {
            cfg.duplicate_near_camera = read_bool_arg(need_value(key));
        } else if (key == "--dedup-large-area-ratio") {
        ...
        // Workload size, MPI schedule, and output controls.
            ...
            cfg.start_frame = std::stoi(need_value(key));
        } else if (key == "--width") {
            cfg.width = std::stoi(need_value(key));
        } else if (key == "--height") {
            cfg.height = std::stoi(need_value(key));
        } else if (key == "--output") {
            cfg.output = need_value(key);
        } else if (key == "--run-id") {
            cfg.run_id = need_value(key);
        } else if (key == "--master-compute") {
            cfg.master_compute = read_bool_arg(need_value(key));
        } else if (key == "--verify") {
            cfg.verify = read_bool_arg(need_value(key));
        } else if (key == "--detector") {
            cfg.detector = need_value(key);
        } else if (key == "--detector-command") {
            cfg.detector_command = need_value(key);
        } else if (key == "--python") {
            cfg.python_bin = need_value(key);
        } else if (key == "--worker-script") {
            cfg.worker_script = need_value(key);
        } else if (key == "--cpu-fallback") {
        ...
        // Live camera controls.
            ...
            cfg.camera_index = std::stoi(need_value(key));
        } else if (key == "--live-video-source") {
            cfg.live_video_source = need_value(key);
        } else if (key == "--camera-script") {
            cfg.camera_script = need_value(key);
        } else if (key == "--viewer-script") {
            cfg.viewer_script = need_value(key);
        } else if (key == "--live-view") {
            cfg.live_view = read_bool_arg(need_value(key));
        } else if (key == "--live-master-compute") {
            ...
            cfg.live_temporal_center = std::stod(need_value(key));
        } else if (key == "--live-temporal-ios") {
    ...
    if (cfg.iou < 0 || cfg.iou > 1 || cfg.duplicate_ios < 0 || cfg.duplicate_ios > 1 ||
        ...
        throw std::runtime_error(
            "--iou, --dedup-ios, --dedup-center, --dedup-axis-overlap, "
            "--dedup-gap, and --dedup-large-area-ratio are out of range"
        );
    }
